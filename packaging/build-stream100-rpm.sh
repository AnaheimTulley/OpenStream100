...
source_archive="$kit_dir/hercules-stream100-0.12.0.tar.gz"
...
mapfile -t installable_rpms < <(
    find "$output_dir" -maxdepth 1 -type f \
        -name 'hercules-stream100-0.12.0-1*.rpm' \
        ! -name '*.src.rpm' \
        ! -name '*-debuginfo-*' \
        ! -name '*-debugsource-*'
)
...
if (( install_after_build )); then
    ...
    echo "Installation complete. Open OpenStream100 from Fedora's app menu."
fi
